...
def _get_pure_code(full_code: str) -> str:
    """从完整代码中获取纯数字代码
    600000.SH -> 600000
    """
...
def _should_filter_position(code: str) -> bool:
    """判断是否应该过滤该持仓
    只同步沪市和深市的股票：
...
def init_auth_system():
    ...
    if CRYPTO_AUTH_CONFIG.get("ENABLED", True):
        ...
        try:
            # 优先使用文件路径配置
            if (
                "PRIVATE_KEY_FILE" in CRYPTO_AUTH_CONFIG
                and "PUBLIC_KEY_FILE" in CRYPTO_AUTH_CONFIG
            ):
                ...
                print(
                    f"加密认证系统已启用（从文件读取密钥: {private_key_file}, {public_key_file}）"
                )

            # 兼容旧的字符串配置方式
            elif (
                "PRIVATE_KEY" in CRYPTO_AUTH_CONFIG
                and "PUBLIC_KEY" in CRYPTO_AUTH_CONFIG
            ):
...
def require_internal_password(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if not InternalPassword.verify_password(password):
            return jsonify({"error": "密码验证失败", "message": "内部密码不正确"}), 401
...
def update_positions():
    try:
        ...
        for pos in data["positions"]:
            if _should_filter_position(pos.get("code", "")):
                print(f"过滤持仓: {pos.get('code', 'unknown')} - 非A股股票")
                continue
            filtered_positions.append(pos)

        print(
            f"更新持仓: {data['strategy_name']}, 原始持仓数: {len(data['positions'])}, 过滤后持仓数: {len(filtered_positions)}"
        )
        total_asset = data.get("total_asset")
        StrategyPosition.update_positions(
            data["strategy_name"], filtered_positions, total_asset=total_asset
        )
        return jsonify(
            {
                "message": "持仓更新成功",
        ...
        return jsonify(
            {
                ...
                "original_positions_count": len(data["positions"]),
...
@app.route("/api/v1/positions/update/total_asset/internal", methods=["POST"])
@require_internal_password  # 使用内部密码验证
def update_total_asset_internal():
    """内部接口：仅更新策略的总资产，不更新持仓数据"""
    try:
        data = request.get_json()
        if not data or "strategy_name" not in data:
            return jsonify({"error": "无效的数据格式，缺少 strategy_name"}), 400

        if "total_asset" not in data:
            return jsonify({"error": "无效的数据格式，缺少 total_asset"}), 400

        StrategyPosition.update_total_asset_only(
            data["strategy_name"], data["total_asset"]
        )
        return jsonify(
            {
                "message": "总资产更新成功（内部接口）",
                "strategy_name": data["strategy_name"],
    ...
    return jsonify(
        {
            ...
            "auth_type": (
                "crypto" if CRYPTO_AUTH_CONFIG.get("ENABLED", True) else "simple"
            ),
            "public_key": (
                CRYPTO_AUTH_CONFIG["PUBLIC_KEY"]
                if CRYPTO_AUTH_CONFIG.get("ENABLED", True)
                else None
            ),
            "algorithm": (
                "RSA-2048" if CRYPTO_AUTH_CONFIG.get("ENABLED", True) else "API-Key"
            ),
...
def get_strategy_positions(strategy_name):
    try:
        ...
        if strategy:
            return jsonify(
                {
                    ...
                    "update_time": (
                        strategy.update_time.strftime("%Y-%m-%d %H:%M:%S")
            ...
            return jsonify({"positions": [], "total_asset": None, "update_time": None})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def get_total_positions():
    try:
                    ...
                    if x_pos > 0 and x_pos < len(strategy_name) - 1:
                        try:
                            ...
                            parsed_strategies.append(
                                {"name": base_name, "coefficient": coefficient}
                            )
                        except ValueError:
                            # 如果解析系数失败，当作普通策略名处理
                            parsed_strategies.append(
                                {"name": strategy_name, "coefficient": 1.0}
                            )
                    else:
                        parsed_strategies.append(
                            {"name": strategy_name, "coefficient": 1.0}
        ...
        result = StrategyPosition.get_total_positions_with_coefficients(
            parsed_strategies, include_adjustments
        )
        return jsonify(
            {
                ...
                "update_time": (
                    result["update_time"].strftime("%Y-%m-%d %H:%M:%S")
...
def refresh_strategies_time():
    ...
    try:
        ...
        return jsonify({"error": f"刷新策略时间失败: {str(e)}", "success": False}), 500
