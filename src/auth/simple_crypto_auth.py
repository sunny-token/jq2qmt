...
def require_auth(f):
    ...
    注意：现在使用基于数据库的加密认证，从数据库的 users 表中读取每个用户的密钥对
    进行验证，而不是使用全局的密钥文件。
    """
    @wraps(f)
    def decorated_function(*args, **kwargs):
        ...
        if CRYPTO_AUTH_CONFIG.get('ENABLED', True):
            return _require_database_crypto_auth(f)(*args, **kwargs)
        else:
            return _require_simple_auth(f)(*args, **kwargs)
    
    return decorated_function

def _require_database_crypto_auth(f):
    """基于数据库的加密认证装饰器
    
    从数据库的 users 表中读取指定用户的 private_key 和 public_key 字段，
    创建临时的 SimpleCryptoAuth 实例进行签名验证。
    
    认证流程：
    1. 解析认证令牌中的 client_id（用户名）
    2. 从数据库查询该用户的密钥对
    3. 使用用户的密钥验证签名
    4. 更新用户的最后活跃时间
    """
    ...
    def decorated_function(*args, **kwargs):
        try:
            ...
            if not client_id:
                return jsonify({'error': '认证令牌中缺少客户端ID'}), 401
            
            # 从数据库获取用户密钥
            from ..models.models import User
            private_key_pem, public_key_pem = User.get_user_keys(client_id)
            
            if not private_key_pem or not public_key_pem:
                return jsonify({'error': f'用户 {client_id} 的密钥未配置或用户不存在'}), 401
            
            # 创建临时的认证实例用于验证
            temp_auth = SimpleCryptoAuth(
            ...
            # 验证令牌
            is_valid, message = temp_auth.verify_auth_token(
            ...
            # 更新用户活跃时间
            User.update_activity_time(client_id)
            
            # 将客户端ID添加到请求上下文
            request.client_id = client_id
            request.auth_type = 'database_crypto'
