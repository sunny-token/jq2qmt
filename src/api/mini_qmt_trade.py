...
def get_xt_trader(path,session_id,account_id,account_type="STOCK"):
    ...
    print("建立交易连接", connect_result)
    if connect_result != 0:
        return False,False
    # 建立交易连接，返回0表示连接成功
    # 对交易回调进行订阅，订阅后可以收到交易主推，返回0表示订阅成功
    subscribe_result = xt_trader.subscribe(acc)
    print("订阅状态", subscribe_result)
    if connect_result==0 and subscribe_result==0:
...
account_id = '8886800503'

path = r'D:\国金证券QMT交易端\userdata_mini'
xt_trader,acc = get_xt_trader(path, session_id, account_id)
if not xt_trader:
    print("Failed to get XtTrader")
    #exit()
...
if __name__ == "__main__":
    ...
    # xt_trader = XtQuantTrader(path, session_id)
    # 创建资金账号为1000000365的证券账号对象
    acc = StockAccount('8886800503', 'STOCK')
    ...
    subscribe_result = xt_trader.subscribe(acc)
    print("订阅状态", subscribe_result)
    stock_code = '002640.SZ'
    # 使用指定价下单，接口返回订单编号，后续可以用于撤单操作以及查询委托状态
    print("order using the fix price:")
    fix_result_order_id = xt_trader.order_stock(acc, stock_code, xtconstant.STOCK_BUY, 20000, xtconstant.FIX_PRICE, 7.50, 'strategy_name', 'remark')
    ...
    for position in positions:
        print(position.account_id, position.stock_code, position.volume)
    print(positions)
